class ParseBoolExpression:
    ...
    def parseBoolExp(self, expression: str) -> bool:
        # Repeteadly simplify the expression by evaluating subexpressions
        while len(expression) > 1:
            start = max(expression.rfind("!"),
                        expression.rfind("&"),
                        expression.rfind("|"))
    ...
    def _evaluate_sub_expr(self, sub_expr:str) -> str:
        ...
        # Apply logical ooperations baed on the operator
        if op == "!":
